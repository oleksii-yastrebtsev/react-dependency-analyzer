...
export default function registerWebView(
  ...
  const disposable = vscode.commands.registerCommand(START, function () {
    const extPath = extensionContext.extensionPath;
    const isDevelopment = process.env.EXTENTION_ENV === "development";
    const hmrHost = isDevelopment ? "http://localhost:3000" : "";
    const hmrHostWS = isDevelopment ? "ws://localhost:3000" : "";
    ...
    const basePath = panel.webview.asWebviewUri(
      vscode.Uri.file(path.join(extPath, "build", "extention"))
    );
    ...
    if (isDevelopment) {
      scriptPath = new URL(`${hmrHost}/webview.js`).toString();
    } else {
